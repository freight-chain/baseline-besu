...
const getReportNode = function getReportNode(node) {
    if (!node)
        return;
    if (node.contentType.includes('multipart/report') &&
...
class AS2Disposition {
    constructor(mdn) {
        ...
        if (mdn) {
            ...
            this.notification = Helpers_1.parseHeaderString(mdn.childNodes[1].content.toString('utf8'), true, toNotificationValue);
