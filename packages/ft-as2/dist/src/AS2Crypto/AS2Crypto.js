...
class AS2Crypto {
  static async buildNode(node) {
    return node.parsed ? await node.build()
  ...
  static async decrypt(node, options) {
    ...
    const recipient =
        p7.findRecipient(forge.pki.certificateFromPem(options.cert));
    if (recipient === null) {
    ...
    const buffer =
        Buffer.from(p7.content.getBytes(), "binary").toString("utf8");
  ...
  static async verify(node, options) {
    ...
    const contentBufferNoCrLf =
        forge.util.createBuffer(AS2Crypto.removeTrailingCrLf(contentPart));
    const signaturePart =
        Buffer.isBuffer(node.childNodes[1].content)
  ...
  static async sign(node, options) {
    ...
    const buffer =
        AS2Crypto.removeTrailingCrLf(await AS2Crypto.buildNode(contentNode));
   ...
   */
  static async compress(node, options) { throw Constants_1.NOT_IMPLEMENTED; }
  /**
   * Not yet implemented.
   * @throws NOT_IMPLEMENTED
   */
  static async decompress(node, options) { throw Constants_1.NOT_IMPLEMENTED; }
}
