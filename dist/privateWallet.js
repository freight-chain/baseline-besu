...
var PrivateWallet = /** @class */ (function(_super) {
  ...
  PrivateWallet.prototype.privateCall = function(transaction) {
    return this.sendPrivateTransaction(transaction)
        ...
        .then(function(receipt) { return receipt.output; });
  };
  PrivateWallet.prototype.signPrivateTransaction = function(transaction) {
    ...
    return properties_1.resolveProperties(transaction).then(function(tx) {
      ...
      var signature = _this._signingKey().signDigest(
          keccak256_1.keccak256(privateTransaction_1.serialize(tx)));
  ...
  PrivateWallet.prototype.populatePrivateTransaction = function(transaction) {
    var _this = this;
    return properties_1.resolveProperties(this.checkTransaction(transaction))
        .then(function(tx) {
          if (tx.to != null) {
            tx.to = Promise.resolve(tx.to).then(function(
                to) { return _this.resolveName(to); });
          }
          if (tx.gasPrice == null) {
            tx.gasPrice = _this.getGasPrice();
          }
          if (tx.nonce == null) {
            tx.nonce = _this.provider.getPrivateTransactionCount(
                _this.getAddress(), transaction);
          }
          ...
          if (tx.from == null) {
            ...
            tx.from =
                Promise
                    .all([
                      _this.getAddress(), _this.provider.resolveName(tx.from)
                    ])
  ...
  PrivateWallet.prototype.checkTransaction = function(transaction) {
    for (var key in transaction) {
      if (allowedPrivateTransactionKeys.indexOf(key) === -1) {
        logger.throwArgumentError("invalid transaction key: " + key,
                                  "transaction", transaction);
      }
    }
